...
from full_scene_generator import generate_full_scene_variant, generate_regenerative_sky_variant
...
def _safe_output_slug(value: str) -> str:
    slug = "".join(ch.lower() if ch.isalnum() else "_" for ch in value.strip())
    slug = "_".join(part for part in slug.split("_") if part)
    return slug or "default"


def _sky_replacement_backend() -> str:
    backend = os.environ.get("PIXELDWELL_SKY_BACKEND", "composite").strip().lower()
    if backend in {"regenerative", "sdxl", "inpaint", "inpainting"}:
        return "regenerative"
    return "composite"


def _sky_result_path(source_path: Path, weather: str, backend: str | None = None) -> Path:
    weather_slug = _safe_output_slug(weather)
    backend_slug = _safe_output_slug(backend or _sky_replacement_backend())
    return OUTPUT_DIR / f"{source_path.stem}_{weather_slug}_{backend_slug}_sky_v23{source_path.suffix}"
...
def _path_to_public_url(path: Path) -> str:
    ...
    try:
        ...
        return f"/output_web/{rel.as_posix()}"
    except ValueError:
        pass

    try:
        rel = path.resolve().relative_to(CLUSTER_OUT.resolve())
        return f"/clusters/{rel.as_posix()}"
    except ValueError:
        pass

    return f"/output_web/{path.name}"


def _generate_sky_replacement(
    input_path: Path,
    output_path: Path,
    weather: str,
    opening_polygons: list[list[list[int]]],
    opening_source_size: tuple[int, int] | None,
) -> Path:
    backend = _sky_replacement_backend()
    if backend == "regenerative":
        return generate_regenerative_sky_variant(
...
def _is_current_output(output_path: Path, input_path: Path) -> bool:
    try:
        return (
            output_path.exists()
...
def _build_enhanced_detection_preview(final_path: Path, original_image_path: Path | None = None) -> str | None:
    ...
    try:
        ...
        if _is_current_output(labeled_path, final_path):
            logging.info("[Detection] Using cached detection preview %s", labeled_path.name)
...
async def enhance_cluster(
    ...
    try:
        ...
        if "sky_replacement" in remaining_features:
            sky_backend = _sky_replacement_backend()
            generated_path = _sky_result_path(out_path, weather, sky_backend)
            opening_polygons, opening_source_size = _resolve_opening_payload(cluster_id=cluster_id)
            logging.info(
                "[EnhanceCluster] sky replacement backend=%s openings=%d source_size=%s",
                sky_backend,
                len(opening_polygons),
                opening_source_size,
            )
            # Never serve a cached result for night or cinematic weathers — always regenerate so lighting changes take effect
            force_regenerate = weather in {"night", "golden_hour", "dusk", "sunset", "sunrise", "dramatic", "partly_cloudy"}
            if not force_regenerate and _is_current_output(generated_path, out_path):
                logging.info("[EnhanceCluster] Using cached sky replacement %s", generated_path.name)
                final_path = generated_path
            else:
                final_path = _generate_sky_replacement(
                    input_path=out_path,
        ...
        if job_id:
            ...
            for feature in ai_features:
                insert_enhancement(
                    ...
                    result_image_path=_path_to_public_url(Path(final_path)),
                    status="completed"
                )
        ...
        return {
            "success": True,
            "image_url": _path_to_public_url(Path(final_path)),
...
async def enhance_single(
    ...
    try:
        ...
        if "sky_replacement" in remaining_features:
            sky_backend = _sky_replacement_backend()
            generated_path = _sky_result_path(source_path, weather, sky_backend)
            opening_polygons, opening_source_size = _resolve_opening_payload(cluster_id=cluster_id, filename=filename)
            logging.info(
                "[EnhanceSingle] sky replacement backend=%s openings=%d source_size=%s",
                sky_backend,
                len(opening_polygons),
                opening_source_size,
            )
            # Never serve a cached result for night or cinematic weathers — always regenerate so lighting changes take effect
            force_regenerate = weather in {"night", "golden_hour", "dusk", "sunset", "sunrise", "dramatic", "partly_cloudy"}
            if not force_regenerate and _is_current_output(generated_path, source_path):
                logging.info("[EnhanceSingle] Using cached sky replacement %s", generated_path.name)
                final_path = generated_path
            else:
                final_path = _generate_sky_replacement(
