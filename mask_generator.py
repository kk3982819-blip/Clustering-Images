...
def _filter_neutral_sky_components(
    ...
    """
    neutral = (neutral_mask > 0).astype(np.uint8) * 255
...
def _find_sky_floor_y(
    ...
    green_land = cv2.inRange(roi_hsv, np.array([25, 18, 25]), np.array([112, 255, 245]))
...
def _extract_sky_from_opening(
    ...
    green_reject = cv2.inRange(roi_hsv, np.array([25, 10, 10]), np.array([105, 255, 235]))
...
def _extract_global_sky_fallback(hsv_img: np.ndarray, gray_img: np.ndarray) -> np.ndarray:
    ...
    green_reject = cv2.inRange(hsv_img, np.array([28, 20, 20]), np.array([95, 255, 220]))
...
def _refine_annotation_sky_mask(
    ...
    green_content = cv2.inRange(hsv, np.array([25, 22, 18]), np.array([108, 255, 245]))
...
def _verify_sky_pixels(img_bgr: np.ndarray, mask: np.ndarray) -> np.ndarray:
    ...
    is_non_sky = cv2.inRange(hsv, np.array([25, 15, 10]), np.array([105, 255, 245]))
